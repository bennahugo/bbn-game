...
namespace BBN_Game.Map
{
    ...
    class MapContent
    {
        ...
        public MapContent(Boolean isTemporary)
        {
            ...
            if (!isTemporary)
            {
                id = Convert.ToString(TempLoader.content.Count);

                for (int i = 0; TempLoader.content.Keys.Contains(id); i++)
                {
                    id = Convert.ToString(TempLoader.content.Count + i);
                }
                TempLoader.content.Add(id, this);
            }
        }
        /// <summary>
        ...
        public bool setNewId(String id)
        {
            if (!isTemporary)
            {
                if (!TempLoader.content.Keys.Contains(id))
                {
                    TempLoader.content.Remove(this.id);
                    this.id = id;
                    TempLoader.content.Add(this.id, this);
                }
                else
                    return false;
            }
        ...
        public void setAttribute(String attName, String newValue)
        {
            ...
            try
            {
                ...
                if (!TempLoader.isObjectInMap(this))
