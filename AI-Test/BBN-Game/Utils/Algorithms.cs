...
namespace BBN_Game.Utils
{
    static class Algorithms
    {
        ...
        /// <summary>
        /// Formula to convert a quaternion to yaw, pitch and roll
        /// Source: http://en.wikipedia.org/wiki/Conversion_between_quaternions_and_Euler_angles
        /// </summary>
        /// <param name="q">input quaternion</param>
        /// <param name="yaw">output yaw</param>
        /// <param name="pitch">output pitch</param>
        /// <param name="roll">output roll</param>
        public static void convertQuaternionToYawPitchRoll(Quaternion q, out float yaw, out float pitch, out float roll)
        {
            ...
            float q2 = q.Y;
            float q3 = q.Z;
            roll = (float)Math.Atan2(2 * (q0 * q1 + q2 * q3), 1 - 2 * (q1 * q1 + q2 * q2));
            pitch = (float)Math.Asin(2 * (q0 * q2 - q3 * q1));
            yaw = (float)Math.Atan2(2 * (q0 * q3 + q1 * q2), 1 - 2 * (q2 * q2 + q3 * q3));
        }
    }
}
